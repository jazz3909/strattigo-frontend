...
export default function CoursePage({
  ...
  // AI content
  const [studyGuide, setStudyGuide] = useState<AiResponse | null>(null);
  const [quiz, setQuiz] = useState<Quiz | null>(null);
  const [studyPlan, setStudyPlan] = useState<AiResponse | null>(null);
  const [aiLoading, setAiLoading] = useState(false);
  const [aiError, setAiError] = useState("");
  const [examDate, setExamDate] = useState("");
  const [studyGuideGeneratedAt, setStudyGuideGeneratedAt] = useState<Date | null>(null);
  ...
  async function handleGenerateStudyGuide() {
    ...
    try {
      ...
      setAiError(err instanceof Error ? err.message : "Failed to generate study guide.");
    } finally {
      setAiLoading(false);
    }
  }
...
function StudyGuideTab({
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-slate-900">Study Guide</h2>
          {generatedAt && !loading && (
            <p className="text-xs text-slate-400 mt-0.5">
        ...
        <div className="flex gap-2">
          {studyGuide && !loading && (
            <Button
              variant="secondary"
              size="sm"
              onClick={() => onCopy(studyGuide.content || "")}
              leftIcon={
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          >
            {studyGuide ? "Regenerate" : "Generate"}
          </Button>
        </div>
      </div>
      ...
      {!loading && !error && !studyGuide && (
        <EmptyState
          ...
          description="Click Generate to create a comprehensive study guide from your course materials."
          action={
      ...
      {!loading && studyGuide && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
          <div
            className="study-guide-content text-sm text-slate-700 leading-relaxed whitespace-pre-wrap"
            dangerouslySetInnerHTML={{ __html: formatMarkdown(studyGuide.content || "") }}
          />
        </div>
      )}
    </div>
